...
#gpu_name = '0,1'
gpu_name = '0'
os.environ['CUDA_VISIBLE_DEVICES'] = gpu_name    # 使用 GPU 0，1

schnetpack_preinstall = False 
if(schnetpack_preinstall):  # qm9 can be download in schnetpack repo
    from schnetpack.datasets import QM9
    # When using QM9 dataset, please make sure to cite the following two papers:
...
class Data_Generater:
    def get_atoms_by_idx(self, index):
        if(schnetpack_preinstall):
            ...
            # logging.debug('Atoms object from schnetpack: %s', at)
        else:  # raw ase db index from 1...
            at = db.get_atoms(id=index+1)
            # logging.debug('Atoms object from raw ase db: %s', at)
        # view(at)
        return at
    ...
    def data_handler(self, idx_lst, a, b, gen_db): # insert db from [a, b)
        ...
        for i in range(a, b):
            for j in range(gen_times):
                ...
                # take care schnet data is starting from 0,
                # but ase db is starting from 1
...
class Agent:
    ...
    def train_UC_FC(self): # satured/unsatured also called fully-coordinate/unfully-coordinate
        self.atoms2matrices_ucfc()
        self.model = keras.Sequential([
            keras.layers.Dense(units=units, #kernel_regularizer=keras.regularizers.l2(0.001),
            ...
            keras.layers.Dense(units=units, #kernel_regularizer=keras.regularizers.l2(0.001),
                               activation=tf.nn.relu),
            #keras.layers.Dropout(rate=0.5),
            # keras.layers.Dense(self.total_clst_cnt, activation=tf.nn.sigmoid)
    ...
    def test_xyz(self):
        ...
        logging.info(
            ...
            + ', MAE: ' + str(mae)\
            + ', MSE: ' + str(mse))
        return [mae], loss # outer compare who is maxest, so here return 1.0/x
    ...
    def test_UC_FC(self):
        ...
        #         if(self.test_labels[idx] == 1 and t[0] > threhold):
        #             a1p1 += 1
        #         elif(self.test_labels[idx] == 0 and t[0] < threhold):
        #             a0p0 += 1
        #         elif(self.test_labels[idx] == 1 and t[0] < threhold):
        #             a1p0 += 1
        #         elif(self.test_labels[idx] == 0 and t[0] > threhold):
        #             a0p1 += 1
        #         else:
        #             bug += 1
        #     logging.info('threhold: ' + str(threhold)\
        #         + 'a1p1:' + str(a1p1)\
        #         + ', a0p0: ' + str(a0p0)\
        #         + ', a1p0: ' + str(a1p0)\
        #         + ', a0p1: ' + str(a0p1)\
        #         + ', bug: ' + str(bug)\
        #         )
        ############## for breakdown of species of BoC
        # for th in range(5, 6):
        #     a1p1 = [0, 0, 0, 0, 0]
        #     a0p0 = [0, 0, 0, 0, 0]
        #     a1p0 = [0, 0, 0, 0, 0]
        ...
        #     for idx, t in enumerate(test_predictions):
        #         atom_type_idx = -1
        #         for ele_idx in range(5):
        #             if self.test_images[idx][ele_idx] == 1:
        #                 atom_type_idx = ele_idx
        #                 break
        #         if atom_type_idx == -1:
        #             print('should not be -1 for atom idx in descriptor?')
        #             1/0 
        #         if(self.test_labels[idx] == 1 and t[0] > threhold):
        #             a1p1[atom_type_idx] += 1
        #         elif(self.test_labels[idx] == 0 and t[0] < threhold):
        #             a0p0[atom_type_idx] += 1
        #         elif(self.test_labels[idx] == 1 and t[0] < threhold):
        #             a1p0[atom_type_idx] += 1
        #         elif(self.test_labels[idx] == 0 and t[0] > threhold):
        #             a0p1[atom_type_idx] += 1
        #         else:
        #             bug += 1
        #     logging.info('threhold: ' + str(threhold)\
        #         + 'a1p1:' + str(a1p1)\
        ...
        #         ti_0.append(self.test_images[i])
        #         tl_0.append(self.test_labels[i])
        #     elif(self.test_labels[i] == 1):
        #         ti_1.append(self.test_images[i])
        #         tl_1.append(self.test_labels[i])
        # logging.info('0 total count is: ' + str(len(tl_0)) + '; 1 total count is: ' + str(len(tl_1)))
        # test_loss, test_acc = self.model.evaluate(np.array(ti_0), np.array(tl_0))
        # logging.info('Test accuracy label 0:' + str(test_acc)\
        #     + ', test loss: ' + str(test_loss))
        # test_loss, test_acc = self.model.evaluate(np.array(ti_1), np.array(tl_1))
        # logging.info('Test accuracy label 1:' + str(test_acc)\
        #     + ', test loss: ' + str(test_loss))

        return test_acc, test_loss

class Agent_2Body_Distance_Mat(Agent): # diagonal 0
    ...
    def fill_labels_atom_saturation(self, row):
        ...
        for i in range(0, len(atoms)-1):
            # if(atoms.get_distance(i, len(atoms)-1) < self.sature_or_not_distance):
            if(atoms.get_distance(i, len(atoms)-1) < self.ucfc_cutoff[atoms[i].symbol]):
                ret.append(0) # not satured
            else:
                ret.append(1)
        return ret
    ...
    def atoms2matrices(self, xyz=False, normalization=True, saturation_judgement=False):
        ...
        # test dataset is too small, in my 28 cpus computer, it will take nearly 10mins,
        # so set pool to 2 is enough
        pool = multiprocessing.Pool()
        if(xyz):
            tmp = pool.map(self.fill_descriptor, rows)
            tmp1 = pool.map(self.fill_labels_xyz, rows)
...
class Agent_BOC(Agent_2Body_Distance_Mat):
    ...
    def generate_cluster_list(self):
        ...
        self.total_clst_cnt += 5 # for H C O F N
    ...
    def fill_descriptor(self, row):
        ...
        # print('fill_descriptor: ', str(atoms), ret)
        return ret
    ...
    def loss(self, x, atoms, unsatured_idx):
        ...
        power = 2
        at = atoms.copy()
        at.append(Atom('H', (x[0], x[1], x[2]))) # element name is no use
        ...
        for i in range(len(at)-1):
            ...
            # to fix: Desired error not necessarily achieved due to precision loss
            if(dis < 0.001):
                dis = 0
            if(abs(dis-d) < 0.001):
                dis = d
            ...
            # cluster_punish += self.cluster_punish_func(dis, at[i].symbol)
        # return unsatured_total_dis + satured_total_dis + cluster_punish
    ...
    def test_xyz(self):
        ...
        # xyz calculated through saturation
        test_predictions = self.model.predict(self.test_images)
        success = 0
        failure_1 = 0
        failure_0 = 0
        ...
        # 1. connect db, cal the descriptor of each atom in atoms
        ...
        # 4. scipy optimizer the loss
                x0 = np.asarray(tuple(mean_xyz))  # 初始猜测值
                res = minimize(self.loss, x0, args=(
                    atoms, unsatured_idx), method='CG') # method='BFGS')
                if(res.success):
                    total_opt_cnt += 3
                    ...
                    # res.x[1] = mean_xyz[1]
                    # res.x[2] = mean_xyz[2]
                    atoms.append(Atom('Au', (res.x[0], res.x[1], res.x[2]))) # element name is no use
                    write('./traj/' + str(row.id)+'.traj', atoms)
                    print('-------------------------------------------------------------')
        ...
        for j in range(len(atoms)):
            for k in range(len(atoms)):
                if(atoms.get_distance(j, k) < 0.00000000000001):
                    ...
                    # 0.529A is bohr radius of hydrogen, maybe we need not mul it
...
class Agent_1D_xyz_pc(Agent_2Body_Distance_Mat): # pc stand for proton numbers
    ...
    def fill_descriptor(self, row):
        ...
        for j in range(len(atoms)):
            ...
            # img[j*4+3] = self.electro_negtive[atoms.numbers[j]]
            # img[j*4+3] = 0.5*(atoms.numbers[j]**2.4)
...
if __name__ == '__main__':
    ...
    for i in range(run_times):
        ...
        for agent in [agent_boc, agent_1dxyz, agent_2body, agent_2body_pc,
                agent_scm, agent_scm_2body, agent_scm_2body_re]: # , agent_electro_neg]:
        # for agent in [agent_boc, agent_2body_pc, agent_scm]: # , agent_electro_neg]:
        # for agent in [agent_electro_counting]: # , agent_electro_neg]:
        # for agent in [agent_boc]: # , agent_electro_neg]:

            # i wrote a train_xyz and test_xyz to predict the x y z of missing atoms
            # but all descriptor give a big error about 1A， i think it is meaningless
            # although some of descriptor is 0.9, some is 1.0, they all too bad...
            # so do not use that case
            if(args.mode == 'type'):
                agent.train()
                acc, loss = agent.test()
                ...
                # agent_boc return 2 acc(mean and opt), so the upper class_type should + 1
                agent.train_xyz()
